...
thread_local std::string              Config::MODEL_NAME = "../models/smoke.engine";

thread_local std::string              Config::BACKBONE = "ResNet50";

thread_local std::string              Config::VIDEO_FILE = "/home/wgf/Downloads/smoke.mp4";

thread_local std::string              Config::RTSP_SITE = "/url/to/rtsp/site";

thread_local std::vector<int>         Config::INPUT_SHAPE = {1, 8, 3, 608, 608};

thread_local std::vector<std::string>  Config::INPUT_NAME = {"im_shape","image","scale_factor"};

thread_local std::vector<std::string> Config::OUTPUT_NAMES = {"dets","num_dets"};
...
thread_local std::string Config::POSTPROCESS_NAME = "SmokeDeployPost";

thread_local std::string Config::POST_TEXT = "Smoke";

thread_local std::string Config::DET_SCORE = "score";
thread_local std::string Config::DET_CLASS_ID = "class_id";
thread_local std::string Config::DET_COORD = "bbox";
thread_local std::string Config::DET_NUM_BBOX = "num_bbox";
...
{	if(init)return;
	...
	if (config["POSTPROCESS"].IsDefined()) {
		...
		if (model_node["DET_SCORE"].IsDefined()) {
			Config::DET_SCORE = model_node["DET_SCORE"].as<std::string>();
		}
		if (model_node["DET_CLASS_ID"].IsDefined()) {
			Config::DET_CLASS_ID = model_node["DET_CLASS_ID"].as<std::string>();
		}
		if (model_node["DET_COORD"].IsDefined()) {
			Config::DET_COORD = model_node["DET_COORD"].as<std::string>();
		}
		if (model_node["DET_NUM_BBOX"].IsDefined()) {
			Config::DET_NUM_BBOX = model_node["DET_NUM_BBOX"].as<std::string>();
		}
	}
	else {
		std::cerr << "Please set MODEL, " << std::endl;
